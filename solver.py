...
#
#   bfs function: breath-first search algorithm, based on the algorithm given in the book.
#
#Required libs: queue (see README for instalation instructions if necessary)
#
...
def generate_child(node):
    ...
    parent = node     #stores the curent state as the parent state
...
def bfs(initialState,goalState):
    ...
    frontier = Queue() #a FIFO queue 
    frontier.put(snode)

    family = [[initialState,None,None]]     #keeps track of all nodes. It is a big list that has smaller lists of children. Necessaryary to backtrace the path.
    explored = []   #keeps track of the explored nodes to avoid recheckig the same node and falling into a loop

    if snode == goal_state:
        return find_path(family)
    
    while not frontier.empty(): 
        node = frontier.get()   #chooses the swallowest node in frontier; the one at the front
        ...
        for child in ch:        #child has the form [[child node],'action',[parent node]]. 
            ...
            if child[0] not in explored:   #traces the child nodes from the list ch that contains all child nodes for a given node
                family.append(child)        #adds the child in the family list
                if child[0] == goalState:   #tests if the child node is the goal state
                           
                    frontier.put(child[0])  #puts child node in the frontier queue; it puts it at the front
                 
                    return find_path(family)    #calls the find_path function
                  
                frontier.put(child[0])      #puts child node in the frontier
...
#it has the form [[[child node],'action',[parent node]],[[child node],'action',[parent node]],...,[[final node],'action',[parent node]]]
    ...
    while parent != None: #only the first state has None as parent
        
        for p in node: #for p in family
    ...
    return steps   #return a list with all the steps
